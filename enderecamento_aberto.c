...
void insere_tabela_hash(char *nome, long long cpf, int nota)
{
    ...
    while (k < TAMANHO_TABELA)
    {
        ...
        if (reg.cpf == cpf)
        {
            // Alterado para não imprimir "CPF já existe" para os 10.000 registros,
            // pois eles devem ser únicos na origem. Isso evita poluição do console.
            // A mensagem é útil para inserções avulsas que podem ser duplicadas.
...
int main()
{
    ...
    printf("\n--- Testando operacoes na tabela_hash.dat (opcional) ---\n");
